...
**v1.0.0** | 42次提交 | 40模块 · 10,732行Python | 568知识库chunks · 10品类 | 12条铁律 · 4人设 | 5种评测格式

---
## 能做什么

```
甲方Brief → 文案生成(4人设+5格式+观点注入) → 14项审核+自动修复 → Docx定稿
                                                       ↓
                                                  分镜生成(9列xlsx+灯位SVG)
                                                       ↓
...
**分镜引擎**：北电教材标准，9列专业xlsx，运镜强制分配(固定≤40%)，灯位SVG俯视图。支持`--columns`(口头描述列)、`--preview`(秒级预览)。

**审核引擎**：14项自动检查(口语化、电商味、流水账、禁用词、态度密度、节奏等)，不合格自动修复(`auto-fix`循环)。`--audience`模式启动AI点映团(模拟目标观众反馈)。
